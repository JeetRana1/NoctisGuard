...
function addRecentGuildEvent(evt) {
  try {
    const now = Date.now(); recentGuildEvents.push({ ...evt, at: now }); // prune older than 10m
    const cutoff = now - (10 * 60 * 1000);
    while (recentGuildEvents.length && recentGuildEvents[0].at < cutoff) recentGuildEvents.shift();
  } catch (e) { }
}
...
function updateBotStats(newStats) {
  ...
  } catch (e) { console.warn('updateBotStats error', e); }
}
    ...
    if (resp && resp.ok && Array.isArray(resp.data.giveaways) && resp.data.giveaways.length) {
      ...
      const norm = (g) => {
        try {
          ...
          if (!out.endsAt) {
            if (out.ends_at) out.endsAt = out.ends_at; else if (out.endAt) {
